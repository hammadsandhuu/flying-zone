...
import faqData from "@/data/umrah-faq.json";

const UmrahByBusPage = () => {
    return (
        <>
            <Breadcrumb pagename="Umrah by Bus" pagetitle="Umrah Packages by Bus" />
                    ...
                    <div className="row">
                        <div className="col-lg-12">
                            <div className="package-inner-title-section mb-40">
                                <h2 className="mb-3">Umrah by air premium</h2>
                                <p>Explore our affordable Umrah packages by air premium</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="col-lg-8">
                            <div className="list-grid-product-wrap mb-70">
                                <div className="row gy-4">
                                    {umrahData.umrahByBus.map((packageItem) => (
                                        <div key={packageItem.id} className="col-md-6 col-lg-4 item">
                                            <div className="package-card">
                                                <div className="package-card-img-wrap">
                                                    <Link
                                                        href={`/umrah/by-bus/${packageItem.slug}`}
                                                        className="card-img"
                                                    >
                                                        ...
                                                        <div className="location">
                                                            ...
                                                            <ul className="location-list">
                                                                {packageItem.location_list.map((location, index) => (
                                                                    <li key={index}>
                                                                        <Link href="/umrah/by-bus">{location}</Link>
                                                                    </li>
                                                                ))}
                                                            </ul>
                                                        </div>
                                                ...
                                                <div className="package-card-content">
                                                    <div className="card-content-top">
                                                        <h5>
                                                            <Link href={`/umrah/by-bus/${packageItem.slug}`}>
                                                                {packageItem.title}
                                                            </Link>
                                                        </h5>
                                                        <div className="location-area">
                                                            <ul className="location-list scrollTextAni">
                                                                {packageItem.place_list.map((place, index) => (
                                                                    <li key={index}>
                                                                        <Link href="/umrah/by-bus">{place}</Link>
                                                                    </li>
                                                                ))}
                                                            </ul>
                                                        </div>
                        ...
                        <div className="col-lg-4">
                            <div className="sidebar-area">
                                ...
                                <h5 className="widget-title mb-30">
                                    FAQ - Umrah by Bus Information:
                                </h5>
                                <div className="faq-content mb-50">
                                    <div className="accordion" id="accordionUmrahBus">
                                        {faqData.umrahByBus.map((faq, index) => (
                                            <div key={faq.id} className="accordion-item">
                                                <h2 className="accordion-header" id={`umrahBusheading${index + 1}`}>
                                                    <button
                                                        className={index === 0 ? "accordion-button" : "accordion-button collapsed"}
                                                        type="button"
                                                        data-bs-toggle="collapse"
                                                        data-bs-target={`#umrahBuscollapse${index + 1}`}
                                                        aria-expanded={index === 0 ? "true" : "false"}
                                                        aria-controls={`umrahBuscollapse${index + 1}`}
                                                    >
                                                        {faq.question}
                                                    </button>
                                                </h2>
                                                <div
                                                    id={`umrahBuscollapse${index + 1}`}
                                                    className={index === 0 ? "accordion-collapse collapse show" : "accordion-collapse collapse"}
                                                    aria-labelledby={`umrahBusheading${index + 1}`}
                                                    data-bs-parent="#accordionUmrahBus"
                                                >
